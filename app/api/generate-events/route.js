...
export async function POST(req) {
    try {
        ...
        const systemPromt = `You are an event planning assistant. Generate event details based on the user's description.
...
- suggestedTicketType should be either "free" or "paid"`
        ...
        const result = await model.generateContent(systemPromt)

        const responce = await result.response
        const text = responce.text()

        // Clean the responce (remove markdown code blocks if present)
        ...
        console.log("Error generating event:", error);
        return NextResponse.json(
